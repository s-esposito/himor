...
class CLIP(Metric):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.model = CLIPModel.from_pretrained(clip_id)
        self.preprocess = CLIPImageProcessor.from_pretrained(clip_id)

        self.add_state("sum_scores", [], dist_reduce_fx="cat")
    
    def __len__(self) -> int:
        return len(self.sum_scores)
    ...
    def update(
        ...
        """Update internal states with CLIP Image scores.
        ...
        preds = F.interpolate(preds, (512, 512), mode="bilinear", align_corners=True)
        targets = F.interpolate(targets, (512, 512), mode="bilinear", align_corners=True)

        preds = self.preprocess.preprocess(preds, do_rescale =False, return_tensors="pt")["pixel_values"].to(preds.device)
        targets = self.preprocess.preprocess(targets,do_rescale =False ,return_tensors="pt")["pixel_values"].to(preds.device)

        with torch.no_grad():
            embedding_pred =self.model.get_image_features(preds)
            embedding_target = self.model.get_image_features(targets)
        
        clip_score = torch.nn.functional.cosine_similarity(embedding_pred, embedding_target)
        self.sum_scores.append(clip_score)
    
    def compute(self) -> torch.Tensor:
        """Compute final CLIP score. """
        return (
            torch.tensor(self.sum_scores, device=self.device)
        ).mean()
