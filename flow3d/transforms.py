...
def rmat_to_cont_6d(matrix: torch.Tensor) -> torch.Tensor:
    """
    :param matrix (*, 3, 3)
    :returns 6d vector (*, 6)
    """
...
def cont_6d_to_rmat(cont_6d: torch.Tensor) -> torch.Tensor:
    """
    :param 6d vector (*, 6)
    :returns matrix (*, 3, 3)
    """
...
def _convert_3x4_to_4x4(trans3x4: torch.Tensor) -> torch.Tensor:
    bottom1x4 = torch.zeros_like(trans3x4[..., :1, :])
    bottom1x4[..., 3] = 1
    trans4x4 = torch.cat([trans3x4, bottom1x4], dim=-2)
    return trans4x4


def compute_relative_transform(node_transforms: torch.Tensor, gaussian_transforms: torch.Tensor) -> torch.Tensor:
    relative_transforms = _convert_3x4_to_4x4(gaussian_transforms) @ _convert_3x4_to_4x4(node_transforms).inverse()
    relative_transforms = relative_transforms[..., :3, :] # (..., 3, 4)
    return relative_transforms


def rt_to_dq(R: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
    q_r = roma.rotmat_to_unitquat(R) 
    q_t = torch.cat([t, torch.zeros_like(t[..., :1])], dim=-1)
    q_d = 0.5 * roma.quat_product(q_t, q_r)
    return torch.cat([q_r, q_d], dim=-1)

def dq_to_rt(dq: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    R = roma.unitquat_to_rotmat(dq[..., :4])
    t = roma.quat_product((2 * dq[..., 4:]), roma.quat_conjugation(dq[..., :4]))[...,:3]
    return R, t

def normalize_dq(dq: torch.Tensor) -> torch.Tensor:
    norm = torch.norm(dq[...,:4], dim=-1, keepdim=True)
    dq = dq / norm
    return dq
